...
<html lang="">
	...
	<!--***INICIA CONTENIDO***-->		
			  ...
			  <div class="row">
					...
					$resultado=mysqli_query($connection,$consulta);
								   ...
								   while ($item=mysqli_fetch_array($resultado)) {
									...
									echo '
									<h3 class="avenir-medium naranja">'.$item['modulo'].'';
        ...
        if (!empty($files)) {
            $filePath = $files[0]; // Se toma el primer archivo encontrado
            
            // Obtener la extensión del archivo
            $fileExtension = pathinfo($filePath, PATHINFO_EXTENSION);
            
            // Definir el nombre de archivo para la visualización (usando el título)
            $displayName = $item2['titulo'] . '.' . $fileExtension;

            // Crear el enlace para abrir en una nueva pestaña
            echo '<a href="javascript:void(0);" onclick="openInNewTab(\'' . $filePath . '\', \'' . $displayName . '\')">
                    <p class="avenir-regular descprov salasub">
                        ' . $item2['titulo'] . '
                    </p>
                  </a>';
...
<style>
	...
	<!--******* VENTANA MODAL AGREGAR*******-->
			...
			<script>
    function openInNewTab(filePath, fileName) {
